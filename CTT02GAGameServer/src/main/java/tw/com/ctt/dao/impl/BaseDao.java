...
public class BaseDao implements Serializable, IBaseDao {
	...
	public String getNewTokenId(String tableName, long accId) {
		...
		try {
			...
			if (this.WRITE_CONN == null || this.WRITE_CONN.isClosed()) {
				...
				sb.append("update " + tableName + " set token_id = ?, online_datetime = DATE_ADD(CURRENT_TIMESTAMP,INTERVAL "
						+ TOKEN_ONLINE_TIME_MINUTE + " MINUTE)");
				sb.append(
						", forced_logout_datetime = DATE_ADD(CURRENT_TIMESTAMP,INTERVAL " + TOKEN_FORCED_LOGOUT_TIME_DAY + " DAY)  where acc_id = ?");
	...
	public boolean checkTokenTimeOut(String tableName, String tokenId, long accId) {
		...
		try {
			...
			if (this.READ_CONN == null || this.READ_CONN.isClosed()) {
				...
				sb.append("where A.online_datetime > CURRENT_TIMESTAMP and A.forced_logout_datetime > CURRENT_TIMESTAMP and A.token_id= ?");
	...
	public Map<String, String> checkTokenTimeOutMember(String tableName, String tokenId, long accId) {
		...
		try {
			...
			if (this.WRITE_CONN == null || this.WRITE_CONN.isClosed()) {
				...
				sb.append(" 	select unix_timestamp(now()) as now_time, count(A.acc_id) as count, B.balance as balance from(	\n");
	...
	public boolean updateTokenTime(String tableName, String tokenId, long accId) {
		...
		try {
			...
			if (this.WRITE_CONN == null || this.WRITE_CONN.isClosed()) {
				...
				sb.append("update " + tableName + " set online_datetime =case when token_id = ? then  DATE_ADD(CURRENT_TIMESTAMP,INTERVAL "
	...
	public boolean getAuthFunction(long accId, String url) {
		...
		try {
			...
			if (this.READ_CONN == null || this.READ_CONN.isClosed()) {
				...
				sb.append("select case when A.acc_level_type = 0 then true when count(C.auth_id) > 0 then true else false end as checkAuth from \n");
				sb.append("(\n");
	...
	private String produceTokenId(long accId) {
		...
		return AESUtil.encrypt_CBC(sb.toString(), sb.toString().substring(sb.toString().length() - 16, sb.toString().length()));
	}
	...
	public boolean checkBlackWhiteIPs() {
		...
		try {
			...
			if (this.READ_CONN == null || this.READ_CONN.isClosed()) {
				...
				sql.append("SELECT UNIX_TIMESTAMP(update_datetime) AS t1 FROM `ctt_manager`.`ctt_black_white_list` ORDER BY update_datetime DESC");
				...
				if (t1 > CommandConstant.LAST_UPDATE_BLACK_WHITE_TIME) {
					sql.append("SELECT CONCAT(ip1,'.',ip2,'.',ip3,'.',ip4) AS ip,type FROM ctt_manager.ctt_black_white_list");
					ps = this.READ_CONN.prepareStatement(sql.toString());
					rs = ps.executeQuery();
	...
	public boolean initServerIPs() {
		...
		try {
			this.checkRead();
			this.checkWrite();
			if (this.READ_CONN == null || this.READ_CONN.isClosed() || this.WRITE_CONN == null || this.WRITE_CONN.isClosed()) {
				LOG.error("no connection");
				return false;
			} else {
				sql = new StringBuilder();
				...
				ps = null;
				if (cou == 0) {
					sql.append("UPDATE `ctt_manager`.`ctt_server` SET status=false WHERE ip=? AND context=? AND port=?");
					ps = this.WRITE_CONN.prepareStatement(sql.toString());
	...
	public void callServer(String url, List<String> notRunList) {
		try {
			if (CommandConstant.SERVERS_IP_LIST != null && !CommandConstant.SERVERS_IP_LIST.isEmpty() && CommandConstant.SERVERS_IP_LIST.size() > 0) {
				String str = "127.0.0.1";
				for (List<String> ll : CommandConstant.SERVERS_IP_LIST) {
					if (notRunList.indexOf(ll.get(1)) == -1) {
						if (CommandConstant.SERVER_IP.equals(ll.get(0)) && CommandConstant.SERVER_CONTEXT.equals(ll.get(1))
								&& CommandConstant.SERVER_PORT.equals(ll.get(2))) {
							str = "127.0.0.1";
						} else {
							str = CommandConstant.SERVER_IP.equals(ll.get(0)) ? "127.0.0.1" : ll.get(0);
							LOG.info("http://" + str + ":" + ll.get(2) + ("".equals(ll.get(1)) ? "/" : (ll.get(1) + "/")) + url + ".php?date="
									+ new Date().getTime());
							touchURL("http://" + str + ":" + ll.get(2) + ("".equals(ll.get(1)) ? "/" : (ll.get(1) + "/")) + url + ".php?date="
									+ new Date().getTime());
						}
					}
				}
