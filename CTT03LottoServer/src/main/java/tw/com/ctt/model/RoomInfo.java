...
public class RoomInfo implements Serializable{
	private static final long serialVersionUID = 3883745525021419075L;
...
//			this.setGameStateChangeTime(System.currentTimeMillis() + this.waitContinueTimeOut * 1000 + 999);
	...
	public Map<String, Long> getFinalResult(){
		...
		if(playerList.size() == 2) {
//			System.out.println(playerList.get(0).getWinTimes()  +"   "+ playerList.get(1).getWinTimes() +"   "+this.getGameTimes());
	...
	public void updateResult() {
		...
		if(playerList.size() == 2) {
			...
			if(winner == 0) {
				...
				gameProcessMap.put("win", p1.getAccId());
				addGameProcessListMap(gameProcessMap);
			}else {
				p2.setWinTimes(p2.getWinTimes()+1);
				gameProcessMap.put("win", p2.getAccId());
				addGameProcessListMap(gameProcessMap);
			}
