...
function showAccountPage(type) {
	...
	if(type == "set" && isMEM(selectedSecMenu)){
		...
		str[7] = '<input type="text" class="margin-fix-2"  id = "inputAccName"  onchange = "checkAccName(this.value);" onkeyup = "this.value = checkAccount(this.value);"> \n';
		...
		str[6] = '<span id = "upAccName"></span><input type="text" class="margin-fix-2"  id = "inputAccName"  onchange = "checkAccName(this.value);" onkeyup = "this.value = checkAccount(this.value);">  \n';
	...
	str[17] = '<input type="text" id = "inputNickName" class="margin-fix-2" maxlength = "'+ACC_NICK_LENGTH_MAX+'" onchange = "checkNickName();" onkeyup = "checkNickName();" >  \n';
	...
	if(isCOM(selectedSecMenu)){
		...
		if(isSC(selectedSecMenu)){
			...
			if(isJSON(getEle("loginInfo").value)){
				...
				if(selectedSecMenu == basic.acc_level_type){
...
function conformDeposit(levelType,originalUpAccBalance){
	...
	if(levelType <= ACC_LEVEL_MEM && levelType >= ACC_LEVEL_SC){
		...
		if(levelType <= ACC_LEVEL_MEM && levelType > ACC_LEVEL_SC){
			 getEle("accNowBalance").value = (parseFloat(getEle("accBalance").value)*1000+parseFloat(depositMoney)*1000)/1000;
			 getEle("upAccBalanceText").innerHTML = (originalUpAccBalance*1000-depositMoney*1000)/1000;
...
function conformWithdrawal(levelType,originalUpAccBalance){
	...
	if(levelType <= ACC_LEVEL_MEM && levelType >= ACC_LEVEL_SC){
		...
		if(levelType <= ACC_LEVEL_MEM && levelType > ACC_LEVEL_SC){
			 getEle("accNowBalance").value = (parseFloat(getEle("accBalance").value)*1000-parseFloat(withdrawalMoney)*1000)/1000;
