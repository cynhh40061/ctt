...
function showOrder() {
	...
	str.push('<div ><button onclick = "onClickOrderRechargeCarrOut();">充值完成訂單</button> \n');
	str.push('<button onclick = "onClickOrderWithdrawalCarryOut();">取款完成訂單</button> \n');
	str.push('<button onclick = "onClickOrderRechargeUndone();">充值未完成訂單</button> \n');
	str.push('<button onclick = "onClickOrderWithdrawalUndone();">取款未完成訂單</button> \n');
	str.push('<button onclick = "onClickOrderReview();">複查訂單</button> \n');
	str.push('</div> \n');
	str.push('<div id = "searchList"></div> \n');
	str.push('<div id = "orderTableList"></div> \n');
...
function showRechargeCarryOut() {
	var json = JSON.parse(getEle("orderJson").value);
	var str = '<div id = "orderSearchTime">'+json.searchTime+'</div><table class="width-percent-910 tr-hover"><tbody><tr><th><a onclick="onChangeOrderSort(0,1);"> \n'
	...
	for (var i = 0; i < Object.keys(josnObject).length; i++) {
		if (josnObject[i].orderId != "") {
			...
			if (order.orderStatus[josnObject[i].status] == '待複查') {
				statusCheck = '<a href="javascript:reviewPassOrder('
						+ josnObject[i].orderId + ',' + 0 + ');">待複查</a>'
			} else {
				statusCheck = order.orderStatus[josnObject[i].status];
			}
			str2 += '<tr><td>'
					+ josnObject[i].orderId
					+ '</td><td>'
					+ josnObject[i].accName
					+ '</td><td>'
					+ josnObject[i].amount
					+ '</td>'
					+ '<td>'
					+ order.rechargeType[josnObject[i].rechargeType]
					+ '</td><td>'
					+ josnObject[i].createDatetime
					+ '</td><td>'
					+ statusCheck
					+ '</td>'
					+ '<td>'
					+ josnObject[i].note
					+ '</td><td>'
					+ (josnObject[i].firstAccName == null ? ''
							: josnObject[i].firstAccName)
					+ '</td><td>'
					+ (josnObject[i].firstDatetime == null ? ''
							: josnObject[i].firstDatetime)
					+ '</td><td>'
					+ (josnObject[i].secondAccName == null ? ''
							: josnObject[i].secondAccName)
					+ '</td><td>'
					+ josnObject[i].backOrderStatus
					+ '</td><td><a href="javascript:checkRechargeCarryOutOrderAjax('
					+ josnObject[i].orderType + ' , ' + josnObject[i].orderId
					+ ');">查看</a></td></tr>';
	...
	if (parseInt(getEle("orderAes").value) == 1) {
		if (!getEle("orderIconArrow" + getEle("orderSort").value).classList
				.contains("icon-arrow-up")) {
			getEle("orderIconArrow" + getEle("orderSort").value).classList
					.add("icon-arrow-up");
		}
	} else {
		if (getEle("orderIconArrow" + getEle("orderSort").value).classList
	...
	for (var i = 0; i < Object.keys(josnObject).length; i++) {
		if (josnObject[i].orderId != "") {
			str2 += '<tr><td>'
					...
					+ josnObject[i].createDatetime
					+ '</td><td>'
					+ order.orderStatus[josnObject[i].status]
					+ '</td>'
					+ '<td>'
					+ josnObject[i].note
					+ '</td><td>'
					...
					+ '</td><td>'
					+ (josnObject[i].secondAccName == null ? ''
							: josnObject[i].secondAccName) + '</td><td>'
					+ josnObject[i].backOrderStatus
					+ '</td><td><a href="javascript:getAuditRechargeOrderAjax('
					+ josnObject[i].orderType + ' , ' + josnObject[i].orderId
					+ ' , ' + josnObject[i].status + ' , '
					+ josnObject[i].rechargeType + ');">受理</a></td></tr>';
...
function showWithdrawalCarryOut() {
	var json = JSON.parse(getEle("orderJson").value);
	var str = '<div id = "orderSearchTime">'+json.searchTime+'</div><table class = "tr-hover">'
	...
	for (var i = 0; i < Object.keys(josnObject).length; i++) {
		if (josnObject[i].orderId != "") {
			...
			str2 += '<tr><td>'
					...
					+ josnObject[i].bank
					+ '</td><td>'
					+ josnObject[i].bankAccName
					+ '</td>'
					+ '<td>'
					+ josnObject[i].bankAcc
					+ '</td><td>'
					+ statusCheck
					+ '</td><td>'
					+ (josnObject[i].firstAccName == null ? ''
					...
					+ '</td><td>'
					+ josnObject[i].type
					+ '</td><td><a href="javascript:checkWithdrawalCarryOutOrderAjax('
					+ josnObject[i].orderType + ' , ' + josnObject[i].orderId
					+ ');">查看</a></td></tr>';
...
function showWithdrawalUndone() {
	var json = JSON.parse(getEle("orderJson").value);
	var str = '<div id = "orderSearchTime">'+json.searchTime+'</div><table class = "tr-hover"><tbody><tr><th><a onclick="onChangeOrderSort(0,4);">單號<i class=\"icon-arrow-down\" id = "orderIconArrow0"></i></a></th>'
	...
	for (var i = 0; i < Object.keys(josnObject).length; i++) {
		if (josnObject[i].orderId != "") {
			str2 += '<tr><td>'
					...
					+ '</td><td>'
					+ josnObject[i].type
					+ '</td><td><a href="javascript:getAuditWithdrawalOrderAjax('
					+ josnObject[i].orderType + ' , ' + josnObject[i].orderId
					+ ' , ' + josnObject[i].status + ' , '
					+ josnObject[i].rechargeType + ');">受理</a></td></tr>';
	...
	for (var i = 0; i < Object.keys(josnObject).length; i++) {
		if (josnObject[i].orderId != "") {
			str2 += '<tr><td>'
					...
					+ '</td>'
					+ '<td>'
					+ order.orderType[josnObject[i].orderType]
					+ '</td><td>'
					+ josnObject[i].createDatetime
					+ '</td>'
					+ '<td>'
					...
					+ (josnObject[i].secondAccName == null ? ''
							: josnObject[i].secondAccName)
					+ '</td><td>'
					+ (josnObject[i].secondDatetime == null ? ''
							: josnObject[i].secondDatetime) + '</td><td>'
					+ josnObject[i].backOrderStatus
					+ '</td><td><a href="javascript:reviewPassOrder('
					+ josnObject[i].orderId + ',' + josnObject[i].orderType
					+ ');">複查</a></td></tr>';
		}
	}
...
function showSearchList() {
	...
	str.push('訂單時間：<input type = "text" id = "orderCreateFirstDatetime" size="10px" maxlength="10" onclick ="newCalendar(this, this.id,1)"  readonly> \n');
	...
	str.push('<select id="changOrderSelectCount"><option value="25">25筆</option><option value="50">50筆</option><option value="75">75筆</option><option value="100">100筆</option></select> \n');
	...
	str.push('<a href="#" onclick="firstOrderPage();" class="backward"></a>  \n ');
	str.push('<a href="#"  onclick = "previousOrderPage();" class="backward-fast"></a>  \n ');
	str.push('<span>總頁數：  \n ');
	str.push('<i id = "displayOrderNowPage"></i>  \n ');
	str.push('<span>/</span><i id = "displayOrderMaxPage"></i>頁</span>  \n ');
	str.push('<a href="#" onclick = "nextOrderPage();" class="forward"></a>  \n ');
	str.push('<a href="#" onclick = "nextOrderMaxPage();" class="forward-fast"></a>  \n ');
	str.push('</li>  \n ');
	str.push('<li class="media-control"><input id="changeOrderPageText" onKeyup = "checkOrderPageText(this.id,this.value);" type="text" class="margin-fix-1">頁');
...
function checkDateTime(){
	...
	if(lastDateTime != null && lastDateTime != undefined && lastDateTime != "" && firstDateTime != null && firstDateTime != undefined && firstDateTime != ""){
...
function onClickOrderReview() {
	...
	var str = '<ul> \n'
			...
			+ '<a href="#" onclick="firstOrderPage();" class="backward"></a> \n'
			+ '<a href="#"  onclick = "previousOrderPage();" class="backward-fast"></a> \n'
			+ '<span>總頁數：<i id = "displayOrderNowPage"></i> \n'
			+ '<span>/</span><i id = "displayOrderMaxPage"></i>頁</span> \n'
			+ '<a href="#" onclick = "nextOrderPage();" class="forward"></a> \n'
			+ '<a href="#" onclick = "nextOrderMaxPage();" class="forward-fast"></a> \n'
			+ '</li> \n'
			+ '<li class="media-control"><input id="changeOrderPageText" onKeyup = "checkOrderPageText(this.id,this.value);" type="text" class="margin-fix-1">頁<button onclick = "changOrderPageBnt( \n'
			+ ');">跳轉</button></li></ul> \n';
...
function getOrderRechargeCarryOutDataAjax() {
	...
	if (typeof XHR != "undefined" && XHR != null) {
		var tmpStr = "tokenId=" + encodeURIComponent(getEle("tokenId").value)
				+ "&count=" + getEle("orderSelectCount").value;
		XHR.timeout = 10000;
		...
		XHR.open("POST", "./Order!getOrderRechargeCarryOutData.php?date="
				+ getNewTime(), true);
		XHR.setRequestHeader("Content-type",
				"application/x-www-form-urlencoded");
		XHR.send(tmpStr + "&page=" + getEle("orderPage").value + "&nextPage="
				+ getEle("orderNextPage").value);
		enableLoadingPage();
			...
			try {
				if (isJSON(XHR.responseText)) {
					...
					if(checkTokenIdfail(json)){
						if (json.orderMaxPage != null
								&& json.orderMaxPage != undefined) {
...
function getOrderRechargeUndoneDataAjax() {
	...
	if (typeof XHR != "undefined" && XHR != null) {
		...
		XHR.open("POST", "./Order!getOrderRechargeUndoneData.php?date="
				+ getNewTime(), true);
		XHR.setRequestHeader("Content-type",
				"application/x-www-form-urlencoded");
		XHR.send(tmpStr + "&page=" + getEle("orderPage").value + "&nextPage="
...
function getOrderWithdrawalCarryOutDataAjax() {
	...
	if (typeof XHR != "undefined" && XHR != null) {
		...
		XHR.open("POST", "./Order!getOrderWithdrawalCarryOutData.php?date="
				+ getNewTime(), true);
		XHR.setRequestHeader("Content-type",
				"application/x-www-form-urlencoded");
		XHR.send(tmpStr + "&page=" + getEle("orderPage").value + "&nextPage="
...
function getOrderWithdrawalUndoneDataAjax() {
	...
	if (typeof XHR != "undefined" && XHR != null) {
		...
		XHR.open("POST", "./Order!getOrderWithdrawalUndoneData.php?date="
				+ getNewTime(), true);
		XHR.setRequestHeader("Content-type",
				"application/x-www-form-urlencoded");
		XHR.send(tmpStr + "&page=" + getEle("orderPage").value + "&nextPage="
...
function getOrderReviewDataAjax() {
	...
	if (typeof XHR != "undefined" && XHR != null) {
		...
		XHR.open("POST", "./Order!getOrderReviewData.php?date=" + getNewTime(),
				true);
		XHR.setRequestHeader("Content-type",
				"application/x-www-form-urlencoded");
		XHR.send(tmpStr + "&page=" + getEle("orderPage").value + "&nextPage="
...
function showMakeUp() {
	...
	for (var i = 0; i <= 23; i++) {
		if (i < 10) {
			selecthrList += '<option value = "0' + i + '">' + '0' + i
					+ '</option>';
		} else {
			selecthrList += '<option value = "' + i + '">' + '' + i
					+ '</option>';
		}
	}
	selectminList = "";
	for (var i = 0; i <= 59; i++) {
		if (i < 10) {
			selectminList += '<option value = "0' + i + '">' + '0' + i
					+ '</option>';
		} else {
			selectminList += '<option value = "' + i + '">' + '' + i
					+ '</option>';
		}
	}
	...
	var str5 = '<tr><th>會員帳號</th><td><input type="text" class="margin-fix-2" id="inputMemberAccName" placeholder="此欄位必填" onchange = "checkMemberAccName(this.value);" maxlength="21"><span id = "checkInputMemberAccNameIcon"></span><span id = "checkInputMemberAccName"></span></td></tr>';
	var str6 = '<tr><th>充值金額</th><td><input type="text" class="margin-fix-2" maxlength="10" id="inputAmount" placeholder="此欄位必填" onkeyup = "checkAmount();" maxlength="20"><span id = "checkInputAmountIcon"></span><span id = "checkInputAmount"></td></tr>';
	var str7 = '<tr><th>帳戶名稱</th><td><input type="text" class="margin-fix-2" id="inputBankAccName" onkeyup = "checkInputBankAccName(this.value);" maxlength="20"></td></tr>';
	var str8 = '<tr><th>卡號</th><td><input type="text" class="margin-fix-2" id="inputBankAcc" onKeyUp="this.value=chkInputBankNo(this.value); maxlength="20"" onMouseOut="this.value=onchgInputBankNo(this.value);" onChange="this.value=onchgInputBankNo(this.value);"><span id = "checkInputBankAcc"></td></tr>';
	var str9 = '<tr><th>銀行</th><td><input type="text" class="margin-fix-2" id="inputBank" onkeyup = "checkInputBank(this.value);" maxlength="20"></td></tr>';
	var str10 = '<tr><th>附言</th><td><input type="text" class="margin-fix-2" id="inputNote" onkeyup = "checkInputNote(this.value);" maxlength="6"></td></tr>';
	var str11 = '<tr><th>銀行單號</th><td><input type="text" class="margin-fix-2" id="inputBankSid" onkeyup = "checkInputBankSid();" maxlength="20"></td></tr>';
	var str12 = '<tr><th>匯款金額</th><td><input type="text" class="margin-fix-2" maxlength="10" id="inputBankCheckAmount" onkeyup = "checkBankAmount();" maxlength="20"><span id = "checkInputBankCheckAmount"></td></tr>';
	var str13 = '<tr><th>銀行到帳時間</th><td><input type="text" class="margin-fix-2" id="inputBankDepositTime" onclick="newCalendar(this, this.id,1)" readonly>'
			+ '<select id="hrList">'
			+ selecthrList
			+ '</select>:'
			+ '<select id="minList">'
	...
	var str16 = '<button class="btn-lg btn-orange" onclick="conformAddOrder();">確定</button><button class="btn-lg btn-gray" onclick="onClickCloseModal();">取消</button>';
	var str17 = '</div>';
	var addorderHtml = "";
	addorderHtml = addorderHtml.concat(str1, str2, str3, str4, str5, str6,
...
function checkMemberAccNameAjax(str) {
	...
	if (typeof XHR != "undefined" && XHR != null) {
		...
		XHR.open("POST", "./Order!checkMemberAccName.php?date=" + getNewTime(),
				true);
		XHR.setRequestHeader("Content-type",
				"application/x-www-form-urlencoded");
		XHR.send(tmpstr);
			...
			try {
				if (isJSON(XHR.responseText)) {
					...
					if (checkTokenIdfail(json)) {
						...
						if (bool == 0) {
							getEle("checkInputMemberAccNameIcon").classList
									.remove("icon-pass");
							getEle("checkInputMemberAccNameIcon").classList
									.add("icon-fail");
							getEle("checkInputMemberAccName").innerHTML = " 此帳號不存在";
...
function conformAddOrder() {
	...
	if (getEle("inputBankDepositTime").value != '') {
		var bank_deposit_datetime = getEle("inputBankDepositTime").value + " "
				+ getEle("hrList").value + ":" + getEle("minList").value;
	} else {
	...
	var str = '';
	str += "member_acc_name=" + member_acc_name + "&amount=" + amount
			+ "&bank_acc_name=" + bank_acc_name + "&bank_acc=" + bank_acc
			+ "&bank=" + bank + "&note=" + note + "&bank_sid=" + bank_sid
			+ "&bank_check_amount=" + bank_check_amount
			+ "&bank_deposit_datetime=" + bank_deposit_datetime + "&remark="
			+ remark;
	if (member_acc_name != ''
			&& amount != ''
			&& remark != ''
...
function searchRechargeCarryOutAjax() {
	...
	var searchAccName = getEle("searchAccName") != undefined
			&& getEle("searchAccName") != null ? getEle("searchAccName").value
			: "";
	var searchAudiAccName = getEle("searchCustomerService") != undefined
			&& getEle("searchCustomerService") != null ? getEle("searchCustomerService").value
			: "";
	var orderCreateFirstDatetime = getEle("orderCreateFirstDatetime") != undefined
			&& getEle("orderCreateFirstDatetime") != null ? getEle("orderCreateFirstDatetime").value
			: "";
	var orderCreateLastDatetime = getEle("orderCreateLastDatetime") != undefined
			&& getEle("orderCreateLastDatetime") != null ? getEle("orderCreateLastDatetime").value
			: "";
	var status = getEle("status") != undefined && getEle("status") != null ? getEle("status").value
			: "";
	...
	if (typeof XHR != "undefined" && XHR != null) {
		var tmpStr = "tokenId=" + encodeURIComponent(getEle("tokenId").value)
				+ "&accName=" + searchAccName + "&status=" + status
				+ "&audiAccName=" + searchAudiAccName + "&firstDatetime="
				+ orderCreateFirstDatetime + "&lastDatetime="
				+ orderCreateLastDatetime 
				+ "&count=" + OrderSelectCount;
		...
		XHR.open("POST", "./Order!searchOrderRechargeCarryOutData.php?date="
				+ getNewTime(), true);
		XHR.setRequestHeader("Content-type",
				"application/x-www-form-urlencoded");
		XHR.send(tmpStr + "&page=" + getEle("orderPage").value + "&nextPage="
...
function searchRechargeUndoneAjax() {
	...
	if (typeof XHR != "undefined" && XHR != null) {
		...
		XHR.open("POST", "./Order!searchOrderRechargeUndoneData.php?date="
				+ getNewTime(), true);
		XHR.setRequestHeader("Content-type",
				"application/x-www-form-urlencoded");
		XHR.send(tmpStr + "&page=" + getEle("orderPage").value + "&nextPage="
...
function searchWithdrawalCarryOutAjax() {
	...
	if (typeof XHR != "undefined" && XHR != null) {
		...
		XHR.open("POST", "./Order!searchOrderWithdrawalCarryOutData.php?date="
				+ getNewTime(), true);
		XHR.setRequestHeader("Content-type",
				"application/x-www-form-urlencoded");
		XHR.send(tmpStr + "&page=" + getEle("orderPage").value + "&nextPage="
...
function searchWithdrawalUndoneAjax() {
	...
	if (typeof XHR != "undefined" && XHR != null) {
		...
		XHR.open("POST", "./Order!searchOrderWithdrawalUndoneData.php?date="
				+ getNewTime(), true);
		XHR.setRequestHeader("Content-type",
				"application/x-www-form-urlencoded");
		XHR.send(tmpStr + "&page=" + getEle("orderPage").value + "&nextPage="
...
function showReviewPassOrder() {
	...
	var str = '<div class="modal-content width-percent-460 margin-fix-5"><span class="close" onclick="onClickCloseModal();">×</span><table class="first-th-fix-1">'
			...
			+ ','
			+ json.orderType
			+ ');">審核通過</button><button class="btn-xl btn-gray" onclick="showAbnormalPage('
			+ json.accId
			+ ','
			+ json.status
			+ ','
...
function showAudiOrder(orderType) {
	...
	str2 += '<h3>'+title+'</h3><table class="first-th-fix-1"><tbody><tr><th>單號</th><td>'
			+ json.orderId + '</td></tr><tr><th>會員帳號</th><td>' + json.accName
			+ '</td></tr>' + '<tr><th>取款金額</th><td>' + json.amount
			+ '</td></tr><tr><th>帳戶名稱</th><td>' + json.bankAccName
			+ '</td></tr><tr><th>卡號</th><td>' + json.bankAcc
			+ '</td></tr><tr><th>銀行</th><td>' + json.bank + '</td>';
	if (orderType == 0) {
		str3 += '</tr><tr><th>附言</th><td>' + json.note + '</td></tr>';
	}
	if (orderType == 0) {
		str4 += '<tr><th>銀行單號</th><td>'
				+ json.bankSid
				+ '</td></tr>'
				+ '<tr><th>匯款金額</th><td>'
				+ json.bankCheckAmount
				+ '</td></tr>'
				+ '<tr><th>銀行到帳時間</th><td>'
				+ (json.bankDepositDatetime == null ? ''
						: json.bankDepositDatetime) + '</td></tr>'
				+ '<tr><th>補單原因</th><td>'
				+ (json.remark == null ? '' : json.remark) + '</td></tr>';
	} else if (orderType == 1 && json.enable == 1) {
	...
	str6 = '<button class="btn-xl btn-orange" onclick="' + passCheck + '(' 
			+ json.status + ',' + json.orderId + ',' + json.orderType
			+ ');">審核通過</button>' + '<button class="btn-xl btn-gray" onclick="'
			+ refuseCheck + '(' + json.status + ',' + json.orderId + ','
...
function auditPassAjax(tmpstr) {
	...
	if (typeof XHR != "undefined" && XHR != null) {
		...
		XHR.setRequestHeader("Content-type",
				"application/x-www-form-urlencoded");
		XHR.send("tokenId=" + encodeURIComponent(getEle("tokenId").value) + "&"
				+ tmpstr);
		enableLoadingPage();
	}
}
...
//	}
	...
	str1 += '<div class="modal-content width-percent-460 margin-fix-5"><span class="close" onclick="onClickCloseModal();">×</span><h3>'+title+'</h3><table class="first-th-fix-1">'
			+ '<tbody><tr><th>單號</th><td>'
			+ json.orderId
			+ '</td></tr><tr><th>訂單狀態</th><td>'
			+ order.orderStatus[json.status]
	...
	str3 += '<tr><th>銀行單號</th><td>'
			+ json.bankSid
			+ '</td></tr><tr><th>匯款金額</th><td>'
			+ json.bankCheckAmount
			+ '</td></tr><tr><th>銀行到帳時間</th><td>'
			+ (json.bankDepositDatetime == null ? '' : json.bankDepositDatetime)
			+ '</td></tr><tr><th>備註</th>'
			+ '<td>'
			+ (json.remark == null ? '' : json.remark)
			+ '</td></tr></tbody></table>'
...
function reviewPassOrder(orderId, orderType) {
	...
	if (typeof XHR != "undefined" && XHR != null) {
		...
		XHR.open("POST", "./Order!checkOrderReview.php?date=" + getNewTime(),
				true);
		XHR.setRequestHeader("Content-type",
				"application/x-www-form-urlencoded");
		XHR.send(tmpStr);
...
function checkRechargeCarryOutOrderAjax(orderType, orderId) {
	...
	if (typeof XHR != "undefined" && XHR != null) {
		...
		XHR.open("POST", "./Order!checkRechargeCarryOutOrderData.php?date="
				+ getNewTime(), true);
		XHR.setRequestHeader("Content-type",
				"application/x-www-form-urlencoded");
		XHR.send(tmpStr);
...
function checkWithdrawalCarryOutOrderAjax(orderType, orderId) {
	...
	if (typeof XHR != "undefined" && XHR != null) {
		...
		XHR.open("POST", "./Order!checkWithdrawalCarryOutOrderData.php?date="
				+ getNewTime(), true);
		XHR.setRequestHeader("Content-type",
				"application/x-www-form-urlencoded");
		XHR.send(tmpStr);
...
function getAuditRechargeOrderAjax(orderType, orderId, status, rechargeType) {
	...
	if (typeof XHR != "undefined" && XHR != null) {
		var tmpStr = "tokenId=" + encodeURIComponent(getEle("tokenId").value)
				+ "&orderType=" + orderType + "&orderId=" + orderId
				+ "&status=" + status + "&rechargeType=" + rechargeType;
		XHR.timeout = 10000;
		XHR.ontimeout = function() {
			disableLoadingPage();
			XHR.abort();
		...
		XHR.open("POST", "./Order!getAuditRechargeOrderData.php?date="
				+ getNewTime(), true);
		XHR.setRequestHeader("Content-type",
				"application/x-www-form-urlencoded");
		XHR.send(tmpStr);
...
function getAuditWithdrawalOrderAjax(orderType, orderId, status, rechargeType) {
	...
	if (typeof XHR != "undefined" && XHR != null) {
		...
		XHR.open("POST", "./Order!getAuditWithdrawalOrderData.php?date="
				+ getNewTime(), true);
		XHR.setRequestHeader("Content-type",
				"application/x-www-form-urlencoded");
		XHR.send(tmpStr);
...
function getSearchDataAjax(page, nextpage) {
	XHR = checkXHR(XHR);
	if (typeof XHR != "undefined" && XHR != null) {
		var tmpStr = getEle("searchOrderInfo").value + "&page=" + page
				+ "&nextPage=" + nextpage;
		XHR.timeout = 10000;
		XHR.ontimeout = function() {
			disableLoadingPage();
...
function rechargePassCheck(status, orderId, orderType) {
	getEle("research").value = "1";
	var json = JSON.parse(getEle("auditOrder").value);
	var tmpstr = "status=" + status + "&orderId=" + orderId + "&orderType="
			+ orderType + "&success=true" + "&memberAccId=" + json.accId
			+ "&orderAmount=" + json.amount
			+ "&remark=&bankSid=&bankCheckAmount=0&bankDepositTime=";
	if (status > 0 && orderId > 0 && orderType >= 0 && json.accId > 0
			&& json.amount > 0) {
...
function rechargeRefuseCheck(status, orderId, orderType) {
	...
	var json = JSON.parse(getEle("auditOrder").value);
	var tmpstr = "status=" + status + "&orderId=" + orderId + "&orderType="
			+ orderType + "&success=false"
			+ "&memberAccId="+json.accId+"&orderAmount=0&remark=" + remark
			+ "&bankSid=&bankCheckAmount=0&bankDepositTime=";
...
function withdrawalPassCheck(status, orderId, orderType) {
	...
	var tmpstr = "status=" + status + "&orderId=" + orderId + "&orderType="
			+ orderType + "&success=true" + "&memberAccId=" + json.accId
			+ "&orderAmount=" + json.amount + "&bankSid=" + bankSid
			+ "&bankCheckAmount=" + bankCheckAmount + "&bankDepositTime="
			+ bankDepositTime + "&remark=";
	if (status > 0 && orderId > 0 && orderType >= 0 && json.accId > 0
			&& json.amount > 0 && bankSid != '' && bankCheckAmount > 0
			&& bankDepositTime != '') {
...
function withdrawalRefuseCheck(status, orderId, orderType) {
	...
	var remark = getEle("refuseReson").value;
	var tmpstr = "status=" + status + "&orderId=" + orderId + "&orderType="
			+ orderType + "&success=false" + "&memberAccId=" + json.accId
			+ "&orderAmount=" + json.amount + "&remark=" + remark
			+ "&bankSid=&bankCheckAmount=0&bankDepositTime=";
	if (status > 0 && orderId > 0 && orderType >= 0 && remark != '') {
		onCheckModelPage('確定送出', 'auditRefuseAjax(\'' + tmpstr + '\')');
	} else {
...
function confromShowOrderLog(upperAction){
	...
	if(upperAction == LOG_ACTION_ADD_RECHARGE_ORDER_CHANGE || upperAction == LOG_ACTION_AUDIT_WITHDRAWAL_ORDER || upperAction == LOG_ACTION_AUDIT_RECHARGE_ORDER || upperAction == LOG_ACTION_AUDIT_ORDER_REVIEW || upperAction == LOG_ACTION_ADD_ORDER){
...
function showOrderLogAjax(str) {
	...
	if (typeof XHR != "undefined" && XHR != null) {
		...
		XHR.open("POST", "./AccountManage!showOrderLog.php?date="
				+ getNewTime(), true);
		XHR.setRequestHeader("Content-type",
				"application/x-www-form-urlencoded");
		XHR.send(tmpStr);
...
function showOrderLog() {
	...
	str[4]='<a href="#" onclick="firstOrderLogPage('+count+');" class="backward"></a>';
	str[5]='<a href="#" onclick="previousOrderLogPage('+count+');" class="backward-fast"></a>';
	str[6]='<span>總頁數：<i id= "displayNowLogPage">1</i><span>/</span><i id = "displayNowLogMaxPage">1</i>頁</span>';
	str[7]='<a href="#" onclick="nextOrderLogPage('+count+');" class="forward"></a>';
	str[8]='<a href="#" onclick="nextOrderLogMaxPage('+count+');" class="forward-fast"></a></p>';
			...
			for(var i = 0; i < addOrderKeys.length; i++){
				for(var j =0;j < detailKey.length;j++){
					if(detailKey[j] == addOrderKeys[i]){
						if(detailKey[j] == 'orderType' || detailKey[j] == 'rechargeType' || detailKey[j] == 'currency' || detailKey[j] == 'status' || detailKey[j] == 'backOrderStatus'){
							data = data.concat('<tr><td>', ORDER_KEYS_LOG_JSON[detailKey[j]],'</td><td>', ORDER_VALUE_LOG_JSON[detailKey[j]][detail[detailKey[j]]], '</td></tr>');
						}else{
...
function nextOrderLogPage(count){
...
function nextOrderLogMaxPage(count){
...
function previousOrderLogPage(count){
...
function firstOrderLogPage(count){
	var nowActionUpperId = getEle("nowActionUpperId").value;
