	...
	public List<ServerInfo> getServerInfo(String context, Boolean status) {
		...
		try {
			...
			if (this.READ_CONN == null || this.READ_CONN.isClosed()) {
				...
				sb.append("select * from ctt_manager.ctt_server where context = ? and status = ?	\n");
				list.add(context);
				list.add(status);

				List<Object> listObj = StmtUtil.queryToBean(this.READ_CONN, sb.toString(), list, new ServerInfo());

				for (int i = 0; i < listObj.size(); i++) {
					serverInfoList.add((ServerInfo) listObj.get(i));
				}
			}
		} catch (Exception e) {
			LOG.debug("Exception, " + e.getMessage());
