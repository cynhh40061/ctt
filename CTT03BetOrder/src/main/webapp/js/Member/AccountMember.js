			...
			try {
				...
				if (isJSON(XHR.responseText)) {
					...
					if(!isNull(json.adData)){
						...
						var jsonAdKey = ["left","leftDown","right","rightDown"];
						var adMidUrlArr = [];
						var adMidSrcArr = [];
						var adMinSrcTextArr = [];
						var gameAdUrlArr = [];
						...
						for(var i = 0 ; i < json.adData.length && !isNull(json.adData[i].adPosition) ; i++){
							...
							else if(adPosition.substring(0,adPosition.length-1) == "adMid"){
						...
						for(var j = 0 ; j < adMidUrlArr.length ; j++){
							if(validateURL(adMidSrcArr[j])){
								...
								if(validateURL(adMidUrlArr[j])){
									...
									adMidStr.push('<a target="_blank" href="#">');
								}
						...
						for(var g = 0 ; g < adMidUrlArr.length ; g++){
							if(validateURL(gameAdSrcArr[g])){
								...
								if(validateURL(gameAdUrlArr[g])){
									...
									gameAdStr.push('<a target="_blank" href="#">');
								}
...
function checkOpenRechargeInfoDiv() {
	...
	if(amount >= 100 && amount <= 1000000){
		...
		for (var i = 0; i <= 23; i++) {
			if (i < 10) {
				addOptionNoDup("hrList","0"+i,"0"+i);
			} else {
				addOptionNoDup("hrList",i,i);
			}
		}
		for (var i = 0; i <= 59; i++) {
			if (i < 10) {
				addOptionNoDup("minList","0"+i,"0"+i);
			} else {
				addOptionNoDup("minList",i,i);
			}
		}
	}else{
		showModelPageType1("系統訊息","請輸入正確金額","確定");
...
function getBankInfoList() {
	...
	for(var i = 0; i < Object.keys(bankCardInfo).length; i++){
		tmpStr = bankCardInfo[i].bankAccName;
		if(tmpStr != ""){
...
function confromSearchGmaeRecords(pageInfo){
	...
	if(!isNull(getEle('gameRecordEndTime').value)){
		if(getEle('gameRecordEndTime').value != ''){
			...
			obj["endTime"] = now.getFromFormat("yyyy/mm/dd") + ' 23:59:59.999';
			getEle('gameRecordEndTime').value = now.getFromFormat("yyyy/mm/dd") + ' 23:59:59.999';
		}
	}
	if(isNull(pageInfo)){
		pageInfo = '';
