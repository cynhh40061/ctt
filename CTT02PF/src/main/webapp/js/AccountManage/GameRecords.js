...
//<input type="hidden" name="gameRecordsData" value="">
//<input type="hidden" name="gameRecordsCount" value="25">
//<input type="hidden" name="gameRecordsPage" value="1">
//<input type="hidden" name="gameRecordsLastPage" value="1">
//<input type="hidden" name="gameRecordsSearchInfo" value="">

function GameRecords_into(){
	var str = '';
	if(document.getElementsByName("gameRecordsData").length!=1){
		str += '\n<input type="hidden" name="gameRecordsData" value="">';
		str += '\n<input type="hidden" name="gameRecordsCount" value="25">';
		str += '\n<input type="hidden" name="gameRecordsPage" value="1">';
		str += '\n<input type="hidden" name="gameRecordsLastPage" value="1">';
...
function showGameRecordsPage(){
	...
	str.push('</span>  \n');
	str.push(' <a href="javascript:void(0);" onclick="firstGameRecordsPage();" class="backward"></a>');
	str.push('<a href="javascript:void(0);" onclick="previousGameRecordsPage();" class="backward-fast"></a>');
	str.push('<span>總頁數：<i id="gameRecordsNowPage">1</i><span>/</span><i id="gameRecordsTotlePage">1</i>頁</span>');
	str.push('<a href="javascript:void(0);" onclick="nextGameRecordsPage();" class="forward"></a>');
	str.push('<a href="javascript:void(0);" onclick="lastGameRecordsPage();" class="forward-fast"></a>');
	str.push('</p>');
	str.push('</div>');
...
function nextGameRecordsPage(){
	...
	var pageInfo = "&count="+getEle("gameRecordsCount").value+"&nextPage="+page;
	confromSearchGmaeRecords(pageInfo);
}
function lastGameRecordsPage(){
	var maxPage = getEle("gameRecordsLastPage").value;
	var pageInfo = "&count="+getEle("gameRecordsCount").value+"&nextPage="+maxPage;
	confromSearchGmaeRecords(pageInfo);
}
...
function firstGameRecordsPage(){
	var pageInfo = "&count="+getEle("gameRecordsCount").value+"&nextPage=1";
	confromSearchGmaeRecords(pageInfo);
}
